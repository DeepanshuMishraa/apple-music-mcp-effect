...
export const listPlaylistsTool = (raw: unknown) =>
  effect.gen(function* () {
...
export const playPlaylistTool = (raw: unknown) =>
  effect.gen(function* () {
...
export const listTracksTool = (raw: unknown) =>
  effect.gen(function* () {
    ...
    const names = yield* osascriptList(
      `tell application "Music" to get name of every track of ${target}`
    );
    const artists = yield* osascriptList(
      `tell application "Music" to get artist of every track of ${target}`
    );
